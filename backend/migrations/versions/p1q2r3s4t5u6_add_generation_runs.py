...
def upgrade() -> None:
    ...
    if "generation_runs" not in existing_tables:
        op.create_table(
            "generation_runs",
    ...
    existing_indexes = {idx["name"] for idx in inspector.get_indexes("generation_runs")}
    if "ix_generation_runs_assessment_id" not in existing_indexes:
        op.create_index(
    ...
    em_columns = {c["name"] for c in inspector.get_columns("evidence_matches")}
    if "run_id" not in em_columns:
        op.add_column(
            "evidence_matches",
    ...
    if "generation_run_id" not in notif_columns:
        op.add_column(
            "notifications",
            sa.Column(
